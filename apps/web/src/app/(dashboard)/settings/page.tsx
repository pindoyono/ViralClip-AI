...
export default function SettingsPage() {
  ...
  const deleteContentProfile = useDeleteContentProfile();
  ...
  const handleCreateProfile = async (e: React.FormEvent) => {
    ...
    await createContentProfile.mutateAsync({
    ...
    <div className="space-y-6">
      ...
      <section className="bg-slate-800 border border-slate-700 rounded-xl p-6 space-y-4">
        ...
        {changePassword.isError && (
          <p className="text-red-400 text-sm">
            {(changePassword.error as Error)?.message ?? "Failed to update password"}
          </p>
        )}
      </section>
      ...
      <section className="bg-slate-800 border border-slate-700 rounded-xl p-6 space-y-4">
        <h2 className="text-white font-semibold text-lg">Content Profiles</h2>
        <p className="text-slate-400 text-sm">
          Content profiles let the AI tailor clip generation to your target audience and platform.
        </p>
              ...
              >
                <div>
                  <p className="text-white text-sm font-medium">
                    {p.name}
                    {p.is_default && (
                      <span className="ml-2 text-xs text-purple-400 font-normal">Default</span>
                    )}
                  </p>
                  <p className="text-slate-400 text-xs capitalize">{p.platform}</p>
                </div>
                <button
                  onClick={() => deleteContentProfile.mutate(p.id)}
                  disabled={deleteContentProfile.isPending}
        ...
        <form onSubmit={handleCreateProfile} className="flex gap-3 flex-wrap">
          <input
            type="text"
            value={newProfileName}
            onChange={(e) => setNewProfileName(e.target.value)}
            placeholder="Profile name (e.g. Tech Reviews)"
            className="flex-1 min-w-[180px] px-3 py-2 bg-slate-700 border border-slate-600 rounded-lg text-white placeholder-slate-400 text-sm focus:outline-none focus:ring-2 focus:ring-purple-500"
          />
          <select
          ...
          <button
            type="submit"
            disabled={createContentProfile.isPending || !newProfileName.trim()}
            className="bg-purple-600 hover:bg-purple-700 disabled:opacity-50 px-4 py-2 rounded-lg text-white text-sm font-medium transition-colors"
          >
            {createContentProfile.isPending ? "Adding…" : "Add Profile"}
          </button>
        </form>
        {createContentProfile.isError && (
          <p className="text-red-400 text-sm">
            {(createContentProfile.error as Error)?.message ?? "Failed to create profile"}
          </p>
        )}
      </section>
